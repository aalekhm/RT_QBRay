															...
															const QBRT::QBRay& vCameraRay)
{
	...
	if (m_bHasNormalMap)
	{
		...
		vNewNormal = PerturbNormal(vNewNormal, pCurrentObject->m_vUVCoords, vUpVector);
	}
	...
	if (m_bHasTexture)
	{
		vDiffuseColour = ComputeDiffuseColour(	vObjectList, 
												...
												vIntersectionPoint, 
												vNewNormal, 
												GetTextureColor(pCurrentObject->m_vUVCoords));
	}
	else
	{
		vDiffuseColour = ComputeDiffuseColour(vObjectList, vLightList, pCurrentObject, vIntersectionPoint, vNewNormal, m_vBaseColour);
	}

	// Compute the reflection component.
	if (m_fReflectivity > 0.0)
	{
		vReflectionColour = ComputeReflectionColour(vObjectList, vLightList, pCurrentObject, vIntersectionPoint, vNewNormal, vCameraRay);
	}
	...
	if (m_fShininess > 0.0)
	{
		vSpecularColour = ComputeSpecularColour(vObjectList, vLightList, vIntersectionPoint, vNewNormal, vCameraRay);
	}
