	...
	if (fIntersectionTest > 0.0)
	{
		...
		else
		{
			...
			if (t1 < t2)
			{
				if (t1 > 0.0)
				{
					vPointOfIntersection = bckRay.m_vPoint1 + (vRayDir * t1);
				}
				else
				{
					if (t2 > 0.0)
					{
						vPointOfIntersection = bckRay.m_vPoint1 + (vRayDir * t2);
					}
					else
					{
						return false;
			...
			vIntersectionPoint = m_GeomTransform.Apply(vPointOfIntersection, QBRT::FWDTFORM);
			...
			qbVector3<double> vNormalVector = vPointOfIntersection;
			vLocalNormal = m_GeomTransform.ApplyNorm(vNormalVector);
			vLocalNormal.Normalize();
			...
			//		            x
			//
			double x = vPointOfIntersection.x;
			double y = vPointOfIntersection.y;
			double z = vPointOfIntersection.z;
