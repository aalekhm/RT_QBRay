												...
												double& fLightIntensity)
{
	...
	for (auto pSceneObject : vObjectList)
	{
		if(pSceneObject != pCurrentObject)
		{
			...
			{
				double fDistance = (sHitData.vPointOfIntersection - vStartPoint).norm();
...
#else
		qbVector3<double> vLightDir;
		{
			vLightDir.x = vLightDir.x;
			vLightDir.y = vLightDir.y;
			vLightDir.z = vLightDir.z;
		}

		// Compute the angle between the local normal and the light ray.
		// Note that we assume that localNormal is a unit vector.
		double angle = acos(qbVector3<double>::dot(vLocalNormal, vLightDir));
