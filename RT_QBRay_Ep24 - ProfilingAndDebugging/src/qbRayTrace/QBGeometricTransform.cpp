...
QBRT::QBRay QBRT::QBGeometricTransform::Apply(const QBRT::QBRay& inputRay, bool bDirFlag)
{
	if (bDirFlag)
	{
		...
		m_TempOutputRay.m_vPoint1 = this->Apply(inputRay.m_vPoint1, QBRT::FWDTFORM);
		m_TempOutputRay.m_vPoint2 = this->Apply(inputRay.m_vPoint2, QBRT::FWDTFORM);
		m_TempOutputRay.m_vDir	= m_TempOutputRay.m_vPoint2 - m_TempOutputRay.m_vPoint1;
	}
	else
	{
		// Apply the backward transform.
		m_TempOutputRay.m_vPoint1 = this->Apply(inputRay.m_vPoint1, QBRT::BCKTFORM);
		m_TempOutputRay.m_vPoint2 = this->Apply(inputRay.m_vPoint2, QBRT::BCKTFORM);
		m_TempOutputRay.m_vDir	= m_TempOutputRay.m_vPoint2 - m_TempOutputRay.m_vPoint1;
	}

	return m_TempOutputRay;
}
