...
QBRT::QBSimpleRefractive::QBSimpleRefractive(	const qbVector4<double>& vColour,
...
qbVector4<double> QBRT::QBSimpleRefractive::ComputeColour(	const std::vector<std::shared_ptr<QBRT::QBObjectBase>>& vObjectList,
															...
															const QBRT::QBRay& cameraRay)
{
	...
	qbVector4<double>	vTranslucentColour;
...
qbVector4<double> QBRT::QBSimpleRefractive::ComputeSpecular(	const std::vector<std::shared_ptr<QBRT::QBObjectBase>>& vObjectList,
																const std::vector<std::shared_ptr<QBRT::QBLightBase>>& vLightList,
																const qbVector3<double>& vIntersectionPoint,
																const qbVector3<double>& vLocalNormal,
																const QBRT::QBRay& cameraRay)
...
qbVector4<double> QBRT::QBSimpleRefractive::ComputeTranslucency(	const std::vector<std::shared_ptr<QBRT::QBObjectBase>>& vObjectList,
																	const std::vector<std::shared_ptr<QBRT::QBLightBase>>& vLightList,
																	const std::shared_ptr<QBRT::QBObjectBase>& pCurrentObject,
																	const qbVector3<double>& vIntersectionPoint,
																	const qbVector3<double>& vLocalNormal,
...
{
	qbVector4<double> vTranslucencyColor;
	
	// Compute the refracted vector.
	// let r = n1/n2
	// c = cos(THETA1) = -vNormal.vIncident
	// vRefracted = r.vIncident + ( r.c - sqrt(1 - r^2(1 - c^2))).vNormal
	qbVector3<double> vIncidentDir = incidentRay.m_vDir;
	vIncidentDir.Normalize();
	qbVector3<double> vTempNormal = vLocalNormal;
	double r = 1.0 / m_fIOR;
	double c = -qbVector3<double>::dot(vTempNormal, vIncidentDir);
	if (c < 0.0)
	{
		vTempNormal = vTempNormal * -1.0;
		c = -qbVector3<double>::dot(vTempNormal, vIncidentDir);
	}
	
	qbVector3<double> vRefracted = r*vIncidentDir + (r*c - sqrtf(1.0 - pow(r, 2.0) * (1.0 - pow(c, 2.0)))) * vTempNormal;
	...
	if (bTest)
	{
		...
		p2.Normalize();
		qbVector3<double> vTempNormal2 = sHitData.vNormalAtPointOfIntersection;
		double r2 = m_fIOR;
		...
		QBRT::QBRay refractedRay2(sHitData.vPointOfIntersection + (vRefractedVector2 * 0.01), sHitData.vPointOfIntersection + vRefractedVector2);
	...
	qbVector4<double> vMaterialColor;
	if (bIntersectionFound)
	{
		// Check if a material has been assigned.
		if (pClosestObject -> m_bHasMaterial)
		{
			vMaterialColor = sClosestHitData.pHitObject->m_pMaterial->ComputeColour(	vObjectList,
																						vLightList, 
