...
void QBRT::QBScene::SetupSceneObjects(uint32_t iEpisode)
{
	switch (iEpisode)
	{
		...
		case 22:	// RT_QBRay_Ep22 - BoundingBoxesAndCompositeShapes
		{
			...
			{
				pSprayBodyMaterial->m_vBaseColour = qbVector4<double>(1.0, 1.0, 1.0, 1.0);
				pSprayBodyMaterial->m_fReflectivity = 0.5;
				pSprayBodyMaterial->m_fShininess = 16.0;
				pSprayBodyMaterial->AssignTexture(sprayTexture);
			}
			...
			QBRT::QBUtils::AddLight(	m_vLightList,	
										MAKE_POSITION(0.0, -20.0, -20.0),
										MAKE_COLOUR(1.0, 1.0, 1.0, 1.0))->m_fLightIntensity = 0.33;
			// Right Light
			QBRT::QBUtils::AddLight(	m_vLightList,	
										MAKE_POSITION(8.0, -20.0, -20.0),
										MAKE_COLOUR(1.0, 1.0, 1.0, 1.0))->m_fLightIntensity = 0.33;
...
{
	// Setup the scene.
	SetupSceneObjects(EPISODE_TO_RENDER);
}
...
bool QBRT::QBScene::Render(QBImage& outputImage)
{
	...
	for (uint32_t y = 0; y < ySize; y++)
	{
		...
		{
			qbVector4<double> vPixelColour = RenderPixel(x, y, xSize, ySize);
...
void QBRT::QBScene::RenderTile(QBRT::DATA::Tile* pTile)
{
	// Loop over each pixel in the tile.
	qbVector4<double> vPixelColor4;
	qbVector3<double> vPixelColor3;

	for (int y = 0; y < pTile->iYSize; ++y)
	{
		for (int x = 0; x < pTile->iXSize; ++x)
		{
			vPixelColor4 = RenderPixel(	pTile->iX + x,
										pTile->iY + y, 
										m_iXSize, m_iYSize);

			uint32_t iPos = Sub2Ind(x, y, pTile->iXSize, pTile->iYSize);

			vPixelColor3.x = vPixelColor4.x;
			vPixelColor3.y = vPixelColor4.y;
			vPixelColor3.z = vPixelColor4.z;

			pTile->vRGBData[iPos] = vPixelColor3;
		}
	}
}

// Function to render an actual pixel.
qbVector4<double> QBRT::QBScene::RenderPixel(int x, int y, int xSize, int ySize)
{
	...
	if (bIntersectionFound)
	{
		...
		if (sClosestHitData.pHitObject->m_bHasMaterial)
		{
			...
			QBRT::QBMaterialBase::m_iReflectionRayCount = 0;
...
int QBRT::QBScene::Sub2Ind(int x, int y, int xSize, int ySize)
{
