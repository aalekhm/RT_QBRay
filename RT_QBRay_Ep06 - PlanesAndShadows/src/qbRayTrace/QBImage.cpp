...
Uint32 QBImage::ConvertColor(const double fRed, const double fGreen, const double fBlue)
{
	...
	unsigned char r = static_cast<char>((fRed / m_fOverallMax) * 255.0f);
	unsigned char g = static_cast<char>((fGreen / m_fOverallMax) * 255.0f);
	unsigned char b = static_cast<char>((fBlue / m_fOverallMax) * 255.0f);
