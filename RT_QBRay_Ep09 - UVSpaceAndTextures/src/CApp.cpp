...
void CApp::RenderTextureTest()
{
	...
	for (uint32_t y = 0; y < SCREEN_HEIGHT; ++y)
	{
		...
		{
			// Compute u,v
			double u = (static_cast<double>(x) / (SCREEN_WIDTH / 2.0)) - 1.0;
			double v = (static_cast<double>(y) / (SCREEN_HEIGHT / 2.0)) - 1.0;
			qbVector2<double> vUVCoords(u, v);

			// Get the colour from the texture.
			qbVector3<double> vPixelColour = qbCheckeredTexture.GetColor(vUVCoords);
