												...
												qbVector3<double>& vLocalColour)
{
	...
	if (fIntersectionTest > 0.0)
	{
		...
		else
		{
			...
			//		            x
			//
			double x = vLocalIntersectionPoint.x;
			double y = vLocalIntersectionPoint.y;
			double z = vLocalIntersectionPoint.z;
			double u = atan(sqrtf(std::pow(x, 2.0) + std::pow(y, 2.0)) / z);
			double v = atan(y / x);
			if (x < 0)
				v += M_PI;
