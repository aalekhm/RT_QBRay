#include "QBCylinder.hpp"
#include <cmath>
#include <array>
#include "qbRayTrace/QBUtils.hpp"

// Default constructor.
QBRT::QBCylinder::QBCylinder()
{
...
bool QBRT::QBCylinder::TestIntersection(	const QBRay& castRay,
											QBRT::DATA::HitData& sHitData)
{
	...
	//	b = 2(PxVx + PyVy)
	//	c = Px^2 + Py^2 - r2
	double a = std::pow(v.x, 2) + std::pow(v.y, 2);
	double b = 2 * ((p.x * v.x) + (p.y * v.y));
	double c = std::pow(p.x, 2) + std::pow(p.y, 2) - 1.0;
	...
	std::array<qbVector3<double>, 4>	vPointsOfIntersection;
	std::array<double, 4>				t;

	bool bT1Valid, bT2Valid; // Collision with the 2 sides of the cylinder.
	bool bT3Valid, bT4Valid; // Collision with the 2 top plates of the cylinder.
	if (fNumSQRT > 0.0)
	{
		...
		if(	t[0] > 0.0
			&&
			fabs(vPointsOfIntersection[0].z) < 1.0
		) {
			bT1Valid = true;
		}
		else
		...
		if(	t[1] > 0.0
			&&
			fabs(vPointsOfIntersection[1].z) < 1.0
		) {
			bT2Valid = true;
		}
		else
	...
	else
	{
		...
		t[3] = (bckRay.m_vPoint1.z + 1.0) / -v.z;

		// Compute the points of intersection.
		vPointsOfIntersection[2] = bckRay.m_vPoint1 + (v * t[2]);
		vPointsOfIntersection[3] = bckRay.m_vPoint1 + (v * t[3]);
		...
		if(	t[3] > 0.0
			&&
			sqrtf(	std::pow(vPointsOfIntersection[3].x, 2.0)	// x^2 + y^2 < 1.0
					+
					std::pow(vPointsOfIntersection[3].y, 2.0))
				< 1.0
		) {
			bT4Valid = true;
		}
	...
	if (!bT1Valid && !bT2Valid && !bT3Valid && !bT4Valid)
		return false;
	...
	if (iMinIndex < 2)
	{
		...
		*/
		qbVector3<double>	vOrigNormal(vValidPointOfIntersection.x, vValidPointOfIntersection.y, 0.0);
		...
		ComputeUV(QBRT::uvCYLINDER, vValidPointOfIntersection, sHitData.vUVCoords);
	...
	else
	{
		...
		if (!CloseEnough(v.z, 0.0))
		{
			...
			if( sqrtf(	std::pow(vValidPointOfIntersection.x, 2.0)	// x^2 + y^2 < 1.0
				...
				//qbVector3<double>	vNormal(0.0, 0.0, 0.0 + vValidPointOfIntersection.z);
				...
				ComputeUV(QBRT::uvPLANE, vValidPointOfIntersection, sHitData.vUVCoords);
