											...
											QBRT::DATA::HitData& sHitData)
{
	...
	// to the plane.
	if (CloseEnough(bckRay.m_vDir.z, 0.0) == false)
	{
		// There is an intersection.
		double t = bckRay.m_vPoint1.z / -bckRay.m_vDir.z;
		...
		if (t > 0.0)
		{
			...
			double U = bckRay.m_vPoint1.x + (bckRay.m_vDir.x * t);
			double V = bckRay.m_vPoint1.y + (bckRay.m_vDir.y * t);
			...
			if ((abs(U) < 1.0) && (abs(V) < 1.0))
			{
				...
				qbVector3<double> vPointOfIntersection = bckRay.m_vPoint1 + (t * bckRay.m_vDir); // => Line EQ
