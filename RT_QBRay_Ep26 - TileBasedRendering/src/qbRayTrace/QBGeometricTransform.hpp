	...
	class QBGeometricTransform
	{
		public:
			...
			void						SetTransform(	const qbVector3<double>& vTranslation,
														const qbVector3<double>& vRotation,
														const qbVector3<double>& vScaling);
			void						SetTransform(const qbMatrix44<double>& matFwd, const qbMatrix44<double>& matBck);
			...
			qbVector3<double>			ApplyNorm(const qbVector3<double>& vInputVector);
		...
		private:
			...
			qbMatrix33<double>			m_matLinearTransform;

			QBRT::QBRay					m_OutputRayInternal;
	};
}