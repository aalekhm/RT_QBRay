...
void QBRT::QBGeometricTransform::SetTransform(const qbMatrix44<double>& matFwd, const qbMatrix44<double>& matBck)
{
	m_matFwd = matFwd;
	m_matBck = matBck;
	ExtractLinearTransform();
...
QBRT::QBRay QBRT::QBGeometricTransform::Apply(const QBRT::QBRay& inputRay, bool bDirFlag)
{
	if (bDirFlag)
	{
		...
		m_OutputRayInternal.m_vPoint1 = this->Apply(inputRay.m_vPoint1, QBRT::FWDTFORM);
		m_OutputRayInternal.m_vPoint2 = this->Apply(inputRay.m_vPoint2, QBRT::FWDTFORM);
		m_OutputRayInternal.m_vDir	= m_OutputRayInternal.m_vPoint2 - m_OutputRayInternal.m_vPoint1;
	}
	else
	{
		// Apply the backward transform.
		m_OutputRayInternal.m_vPoint1 = this->Apply(inputRay.m_vPoint1, QBRT::BCKTFORM);
		m_OutputRayInternal.m_vPoint2 = this->Apply(inputRay.m_vPoint2, QBRT::BCKTFORM);
		m_OutputRayInternal.m_vDir	= m_OutputRayInternal.m_vPoint2 - m_OutputRayInternal.m_vPoint1;
	}

	return m_OutputRayInternal;
}
...
qbVector3<double> QBRT::QBGeometricTransform::ApplyNorm(const qbVector3<double>& inputVector)
{
	// Apply the transform and return the result.
	qbVector3<double> result = m_matLinearTransform * inputVector;
	return result;
}
...
void QBRT::QBGeometricTransform::ExtractLinearTransform()
{
	...
	for (int i=0; i<3; ++i)
	{
		for (int j=0; j<3; ++j)
		{
			m_matLinearTransform.SetElement(i, j, m_matFwd.GetElement(i, j));
		}
	}
	
	// Invert and transpose.
	m_matLinearTransform.Inverse();
	m_matLinearTransform = m_matLinearTransform.Transpose();
	
}

// Function to return the normal transform.
qbMatrix33<double> QBRT::QBGeometricTransform::GetNormalTransform()
{
	return m_matLinearTransform;
}
