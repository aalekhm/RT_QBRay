...
qbVector3<double> QBRT::QBTexture::QBImageTexture::GetColourAtPosition(uint32_t x, uint32_t y)
{
	qbVector3<double> vOutPixelColour(1.0, 0.0, 1.0);
	...
	if (((x >= 0) && (x < m_iWidth) && (y >= 0) && (y < m_iHeight)))
	{
		...
		SDL_GetRGB(iPixel, m_pPixelFormatDetails, nullptr, &iRed, &iGreen, &iBlue);

		vOutPixelColour.x = iRed * ONE_OVER_255;
		vOutPixelColour.y = iGreen * ONE_OVER_255;
		vOutPixelColour.z = iBlue * ONE_OVER_255;
	}
...
qbVector3<double> QBRT::QBTexture::QBImageTexture::GetColor(const qbVector2<double>& vUVCoords)
{
	qbVector3<double> vOutPixelColour(1.0, 0.0, 1.0);
